...
@app.route('/traveltimes')
def show_traveltimes():
    travel_times = database.list_traveltimes()
    if travel_times is None:
        flash("Error fetching travel times.")
    page['title'] = 'Travel Times'
    return render_template('show_travel_times.html', page=page, session=session, travel_times=travel_times)

@app.route('/search_paths_bystops', methods=['POST'])
def search_paths_bystops():
    try:
        stop_number = int(request.form.get('searchterm'))
        print("the number you search is "+str(stop_number))
    except ValueError:
        flash("Invalid stop number entered.")
        return redirect(url_for('index'))

    paths = database.search_paths_bystops(stop_number)
    print("Do we have the path? "+str(paths))
    if not paths:
        flash(f"No paths found for stop number: {stop_number}")
        return redirect(url_for('index'))
    
    page['title'] = f'Paths for stop number: {stop_number}'
    return render_template('show_travel_times.html', page=page, session=session, travel_times=paths)

@app.route('/add_travel_time', methods=['POST', 'GET'])
def add_travel_time():
    if request.method == 'POST':
        startstationid = request.form['startstationid']
        endstationid = request.form['endstationid']
        expectedtraveltimeseconds = request.form['expectedtraveltimeseconds']
        stopstraversed = request.form['stopstraversed']
        triplegs = request.form['triplegs']
        coordinatemaplen = request.form['coordinatemaplen']
        success = database.insert_travel_time(startstationid, endstationid, expectedtraveltimeseconds, stopstraversed, triplegs, coordinatemaplen)
        if success:
            flash("Travel time added successfully!")
            return redirect(url_for('show_traveltimes'))
        else:
            flash("Error adding travel time.")
    page = {'title': 'Add Travel Time'}
    station_ids = database.list_station_ids()
    return render_template('add_travel_time.html', page=page, session=session, station_ids=station_ids)

@app.route('/delete_travel_time/<startstationid>/<endstationid>', methods=['GET'])
def delete_travel_time(startstationid, endstationid):
    success = database.delete_travel_time(startstationid, endstationid)
    if success:
        flash("Travel time deleted successfully!")
        return redirect(url_for('show_traveltimes'))
    else:
        flash("Error deleting travel time.")
        return redirect(url_for('show_traveltimes'))

@app.route('/update_travel_time_form/<int:startstationid>/<int:endstationid>', methods=['GET'])
def update_travel_time_form(startstationid, endstationid):
    travel_time = database.get_travel_time(startstationid, endstationid)
    if not travel_time:
        flash("Travel time not found.")
        return redirect(url_for('list_travel_times'))
    
    print("This is travel_time: "+str(travel_time))

    return render_template('update_travel_time_form.html', item=travel_time, page={"title": "Update Travel Time"}, session=session)

@app.route('/update_travel_time', methods=['POST'])
def update_travel_time():
    startstationid = int(request.form['startstationid'])
    endstationid = int(request.form['endstationid'])
    expectedtraveltimeseconds = int(request.form['expectedtraveltimeseconds'])
    stopstraversed = int(request.form['stopstraversed'])
    triplegs = int(request.form['triplegs'])
    coordinatemaplen = int(request.form['coordinatemaplen'])


    success = database.update_travel_time_in_db(
        startstationid,
        endstationid,
    ...
    if success:
        flash('Travel time updated successfully!')
        return redirect(url_for('show_traveltimes'))
    else:
        flash('Error updating travel time.')
        return redirect(url_for('update_travel_time_form', startstationid=startstationid, endstationid=endstationid))

@app.route('/produce_report')
def produce_report():
